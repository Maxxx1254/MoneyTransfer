...
public class DashboardPage {
    ...
    private static final ElementsCollection cards = $$("[class='list__item']");
...
//    UserData user = new UserData("5000");
    ...
    public static int getLastCardBalance() {
        val text = cards.last().text();
        return extractBalanceLastCard(text);
    }

    private static int extractBalanceLastCard(String text) {
        val start = text.indexOf(balanceStart);
        val finish = text.indexOf(balanceFinish);
        val value = text.substring(start + balanceStart.length(), finish);
        return Integer.parseInt(value);
    ...
    public static int transferMoneyOnFirstCard(UserData.PayInfo payInfo) {
        ...
        numberCard.setValue(payInfo.getCardLast());
        pay.click();
        heading.should(visible);
        return getFirstCardBalance();
    }
    public static int transferMoneyOnLastCard(UserData.PayInfo payInfo) {
        popolnit.last().click();
        amount.setValue(payInfo.getAmount());
